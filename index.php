...
class Imdb
{
  ...
  private function scrapeMovieInfo($imdbUrl, $getExtraInfo = true)
  {
	...
	$srplit=explode("<hr>",explode("iv>",$ssplit)[0])[1];
	$split=explode("<div>",explode("<\/d",$srplit)[0])[1];
	}
	else
	{
	$ssplit=explode("<section class=\"titlereference-section-overview\">",explode("iv>",$html)[0])[1];
	$split=explode("<div>",explode("<\/d",$ssplit)[0])[1];
	}
	$score=$arr['rating']*10;
	if($score==100)
